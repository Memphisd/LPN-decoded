...
//Hashmap
//first = current size of a bucket, second = max size of a bucket
std::pair<int, int> ** bucket_meta = new std::pair<int, int>*[threads];
int el_per_bucket = (permutations / (1 << r1)) * 3;
std::vector<std::vector<std::vector<std::pair<int, int>>>> Hashmap(threads, std::vector<std::vector<std::pair<int, int>>>(r1 > r2 ? 1 << r1 : 1 << r2, std::vector < std::pair<int, int >> (el_per_bucket)));
...
{
	//Hash
	int max = indices == 2 ? 1 << r1 : 1 << r2;
	for (int i = 0;i < max;++i)
		bucket_meta[thread][i].first = 0;
	...
	for (int i = 0;i < L1_size;++i)
	{
		...
		cur_buc = (*L1)[i].value[0] >> (64 - shift);

		if (bucket_meta[thread][cur_buc].first >= bucket_meta[thread][cur_buc].second - 1)
		{
			std::pair<int, int> bucket_el;
			Hashmap[thread][cur_buc].push_back(bucket_el);
			bucket_meta[thread][cur_buc].second++;
		}
		Hashmap[thread][cur_buc][bucket_meta[thread][cur_buc].first].first = (*L1)[i].index[0];
		if (indices == 4)
			Hashmap[thread][cur_buc][bucket_meta[thread][cur_buc].first].second = (*L1)[i].index[1];

		bucket_meta[thread][cur_buc].first++;
	...
	for (int i = 0;i < L2_size;++i)
	{
		...
		cur_buc = (*L2)[i].value[0] >> (64 - shift);
		

		for (int j = 0;j < bucket_meta[thread][cur_buc].first;++j)
		{
			if (indices == 2)
			{
				...
				(*merged_List)[c].index[0] = Hashmap[thread][cur_buc][j].first;
				(*merged_List)[c].index[1] = (*L2)[i].index[0];
			}
			else
				if (check_solution(Hashmap[thread][cur_buc][j].first, (*L2)[i].index[0], Hashmap[thread][cur_buc][j].second, (*L2)[i].index[1], L_h, L_l, Q_trans, syn_tilde, perm_mat,thread))
				{
					temp[thread] = true;
					break;
				}
...
void start_search(list L_h, list L_l, uint64_t ** parity_mat , uint64_t * syndrom, bool * stop)
{
	...
	int buckets = r1 > r2 ? 1 << r1 : 1ULL << r2;

	//store bucket meta information
	bucket_meta[thread] = new std::pair<int, int>[buckets];
	for (int i = 0;i < buckets;++i)
	{
		bucket_meta[thread][i].first = 0;
		bucket_meta[thread][i].second = el_per_bucket;
	}
